  ...
  ({ label, error, helperText, className = '', id, ...props }, ref) => {
    ...
    return (
      <div className="w-full">
        {label && (
          <label htmlFor={inputId} className="block text-sm font-semibold text-text dark:text-gray-200 mb-2">
            {label}
        ...
        />
        {error && (
          <p id={errorId} className="mt-2 text-sm text-error dark:text-error-light flex items-center gap-1.5" role="alert">
        ...
        {helperText && !error && (
          <p id={helperId} className="mt-2 text-sm text-text-secondary dark:text-gray-400">{helperText}</p>
        )}
      </div>
    );
  }
